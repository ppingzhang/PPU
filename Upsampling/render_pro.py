...
from drc import drc_projection
import math
import numpy as np
...
def render_views(pc, batch_size, p_rgb= None):
	...
	ss = x_rot.shape
	print(ss[0],'-----')
	trams = np.zeros([ss[0],4])
	for ii in range(ss[0]):
		rotation_matrix = euler2mat(x_rot[ii], y_rot[ii], z_rot[ii])
		trams[ii,:] = mat2quaternion(rotation_matrix)
...
def render_views_8(pc, batch_size, p_rgb= None):

	x_rot = np.array([  0, -90, 180, 90,  0,   0])
	y_rot = np.array([  0, 180, 0,    0, 90, -90])
	z_rot = np.array([-90,   0, 90,   0,  0, 180])
