...
def chamfer_for(point, label, radius=1.0):
    dists_forward, _, dists_backward, _ = tf_nndistance.nn_distance(point, label)
    CD_dist = 0.5 * dists_forward
    CD_dist = tf.reduce_mean(CD_dist, axis=1)
    CD_dist_norm = CD_dist / radius
    cd_loss = tf.reduce_mean(CD_dist_norm)
    return cd_loss

def normal_loss(point, label, radius=1.0):

    #f_idx=[B,Npoint]
    #b_idx = [B, Nlabel]
    #batch_size = point.get_shape()[0].value
    #dists_forward, f_idx, dists_backward, b_idx = tf_nndistance.nn_distance(point, label)

    
    dist = tf.abs(point - label)
