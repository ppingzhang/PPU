...
def rotate_point_cloud_and_gt_and_gt_x2(batch_data,batch_gt=None,batch_gt_x2=None,z_rotated=True):
    """ Randomly rotate the point clouds to augument the dataset
        rotation is per shape based along up direction
        Input:
          BxNx3 array, original batch of point clouds
    ...
    for k in range(batch_data.shape[0]):
        ...
        if batch_gt_x2 is not None:
            batch_gt_x2[k, ..., 0:3]   = np.dot(batch_gt_x2[k, ..., 0:3].reshape((-1, 3)), rotation_matrix)
            if batch_gt_x2.shape[-1] > 3:
                batch_gt_x2[k, ..., 3:] = np.dot(batch_gt_x2[k, ..., 3:].reshape((-1, 3)), rotation_matrix)

    return batch_data,batch_gt, batch_gt_x2
...
def random_scale_point_cloud_and_gt_and_gt_x2(batch_data, batch_gt = None, batch_gt_x2 = None, scale_low=0.5, scale_high=2):
    """ Randomly scale the point cloud. Scale is per point cloud.
        Input:
            BxNx3 array, original batch of point clouds
        Return:
    ...
    if batch_gt_x2 is not None:
        for batch_index in range(B):
            batch_gt_x2[batch_index, :, 0:3] *= scales[batch_index]


    return batch_data,batch_gt,batch_gt_x2,scales
